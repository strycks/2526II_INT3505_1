...
class UserList(Resource):
    ...
    def get(self):
        ...
        # Database index is saved as tree, traversing it takes O(logn). 
        # Here I use binary search which is also O(logn) to better demonstrate (shorter code, easier to follow).
        l = -1
        r = len(users_db)
        while r - l > 1:
            database_delay()
            mid = (l + r) // 2
            if users_db[mid]['id'] >= after_id:
                r = mid
            else:
                l = mid
            
        start_idx = r
        if start_idx == len(users_db): 
